...
class NotificationService
{
    public function __construct(
        ...
        #[Autowire(env: 'FIREBASE_SERVER_KEY')]
        private string $firebaseServerKey = '',
        #[Autowire(env: 'PUSHER_APP_ID')]
        private string $pusherAppId = '',
        #[Autowire(env: 'PUSHER_KEY')]
        private string $pusherKey = '',
        #[Autowire(env: 'PUSHER_SECRET')]
        private string $pusherSecret = '',
        #[Autowire(env: 'PUSHER_CLUSTER')]
        private string $pusherCluster = 'eu',
    ) {
        // Vérification des clés requises
        if (empty($this->firebaseServerKey)) {
            throw new \RuntimeException('La clé Firebase n\'est pas configurée. Vérifiez vos variables d\'environnement.');
        }
        
        if (empty($this->pusherKey) || empty($this->pusherSecret)) {
            throw new \RuntimeException('Les identifiants Pusher ne sont pas correctement configurés. Vérifiez vos variables d\'environnement.');
        }
    }
